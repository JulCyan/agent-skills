...
- [ ] 所有自动化测试只使用合成 fixture、fake adapter 或 fake Shopify CLI；不得读取
  `.env`、token、cookie、凭据或调用真实 Shopify。
- [ ] 不修改或安装到 `shopify-theme-next`，不让新 Skill 依赖
  `shopify-theme-multi`。
- [ ] 不实现 theme create/duplicate、Live target override、Publish、隐式 target、任意
  JSON Pointer 或 Git 代码同步。
- [ ] 每项行为先写失败测试，再写最小实现；实现阶段保留可复查的 RED/GREEN 命令证据。
- [ ] 只在本 change 的功能分支提交；不 push、PR、Release、Publish 或真实写入。
- [ ] 规格 commit 与实现 commit 独立；实现默认汇总为一个可验证、可回滚的本地 commit，
  不按文件类型机械拆分。
...
- [ ] 将 discovery 预期从一个 Skill 改为两个，并分别断言
  `shopify-media-sync` 与 `theme-template-sync`，继续排除 `openspec-*`。
- [ ] 扩展 tracked-snapshot copy-install 测试：断言安装两个 Skill、两个 lock entry 的
  SHA 格式和 `verifyInstalledSkill(...).status === "MATCH"`；新 Skill 只运行 `--help`，
  不执行远端命令。
- [ ] 为新 POSIX launcher 写仓库外 cwd 测试：显式 fake binary 接收原样 argv 且 cwd
  不变；`PATH` 中无 Go 且未指定 binary 时 stdout 为
...
- [ ] 将 `test:go` 扩展为依次测试两个独立 Go module；将 `test:shell` 扩展为对两个
...
- [ ] 先测试 `--help` exit 0、未知 command/flag exit 2、stdout 只含机器结果、stderr
  只含诊断，且 dependency 未建立时不得 panic。
- [ ] 建立 `module theme-template-sync`、`go 1.25.1`，不增加第三方依赖。
- [ ] `main` 使用 `signal.NotifyContext`，只装配 production dependencies、调用一次
  `app.Run` 并 `os.Exit`；context 不存入 struct。
- [ ] wrapper 按“可执行的 `THEME_TEMPLATE_SYNC_BINARY` -> bundled Go source ->
  structured `NEEDS_SETUP`”选择运行时。source fallback 以内部
  `THEME_TEMPLATE_SYNC_CALLER_CWD` 传递原调用目录，并在 module cwd 执行 `go run`；
  CLI 对用户路径仍以原调用目录解析。
- [ ] 运行 `go test ./cmd/theme-template-sync ./internal/app` 和
...
- [ ] 先用 table-driven tests 覆盖 `page.example`、`product.lp4`、
  `product.lx2_42w`、`collection.example`、deprecated `--page` alias、双入口冲突、
  slash/backslash/absolute/`..`/`.json`/`templates/` 注入。
- [ ] 实现只返回 `templates/<key>.json` 的 `TemplateKey`；warning 只用于 alias，不建立
  Page 专属模型。
- [ ] 先测试 BOM、前导空白、连续 Shopify block/line comments、正文 comment、尾随垃圾、
  第二 root value、非 object root 与 JSON parse error。
- [ ] 用 `json.Decoder.UseNumber()` 解析恰好一个 root object；只剥离前导 comment，保存
  target Header，拒绝 JSON5 其它语法。
- [ ] 先测试未知顶层字段、精确 number token、object key 顺序无关、array 顺序敏感和
  canonical hash 重复运行稳定。
- [ ] 实现通用 `map[string]any` clone 与 canonical encoding；object keys 排序、array
  保序、`json.Number` 不转 float，canonical 输出为单个 UTF-8 JSON object 加换行。
- [ ] 先测试 `sections` 非 object、`order` 非 string array、重复 key、超过 25 项；实现
  `Validate`，并对 source、target、planned 复用同一验证。
- [ ] 运行 `go test -count=1 ./internal/templatejson`，确认全部 GREEN。
...
- [ ] 先测试完整模式 planned canonical 完全等于 source，包含未知顶层字段，且 target
  comment/格式差异不影响语义；target read 不存在由 adapter/preflight fail closed。
- [ ] 先测试 Section 已存在时只替换该 Section、其它 Section/未知字段/order 不变；
  缺失时按 source 最近可用前驱插入，找不到前驱时追加，existing Section 不在 order 时
  补入，超过 25 项失败。
- [ ] 先测试 field 模式只接受 `custom_css`；source/target Section 或 field 缺失、
  `null`、string、object、number、mixed array 均报路径化错误。
- [ ] 实现 field 模式只替换目标 `[]string`，并用 canonical before/after 断言目标 Section
  的 settings、blocks、其它字段、其它 Sections、order 与未知顶层字段不变化。
- [ ] 先构造同时含 added/removed/changed/order/Section summary 的 fixture，连续运行两次并
  断言 marshaled diff 字节相同；只有 comment/format/key order 差异时 `Diff.Empty()`。
- [ ] diff path 使用稳定的 dot/index 表达，object key 排序遍历；`order` 单独输出 before、
  after、added、removed、moved，避免与通用 array diff 重复。
- [ ] `HasRemoval` 覆盖 object/Section/array/order 删除；`custom_css` 数组缩短必须为
  removal，scalar 内容替换只算 changed。
- [ ] 运行 `go test -count=1 ./internal/syncengine`，确认全部 GREEN。
...
- [ ] 先测试 store/theme 空值和不安全 control/path 输入在 runner 前拒绝；合法参数始终作为
  独立 argv，不经 `sh -c`。
- [ ] FakeAdapter 记录单调序号的 resolve/read/write calls，并支持按 target/call 注入
  resolve/read/write failure、role drift 和 readback body；测试可断言 mutation count、
  最大并发与调用顺序。
- [ ] 用 fake runner 先测试 theme list JSON 的 ID string/number、精确 ID、唯一精确 name、
  零命中、名称多命中、未知/空 role 和 CLI failure。
- [ ] 实现 list argv 仅为 `theme list --store <store> --json`；规范化 Live 判断时大小写
  不敏感，但不猜测主题。
- [ ] 用临时目录测试 pull argv 含唯一 asset path、读取缺失文件返回 typed not-found；push
  argv 仅含 `theme push --path <tmp> --store <store> --theme <id> --only
  templates/<key>.json --nodelete --json`。
- [ ] 断言 production adapter 从不生成 create/duplicate/publish/`--live`/`--allow-live`/
  `--unpublished` flag，也不读取 `.env`。
- [ ] 运行 `go test -count=1 ./internal/adapter`，确认全部 GREEN。
...
- [ ] 先测试默认 root 从 caller cwd 形成
  `.runtime/theme-template-sync/<UTC timestamp-random run-id>`，run-id 不接受用户输入。
- [ ] 先测试绝对相对路径、`..`、空 segment、root 内 symlink file/dir 与已有 symlink
  manifest 均 fail closed，root 外 victim 保持不变。
- [ ] 使用 Go 1.25 `os.OpenRoot`/`os.Root` 约束所有 artifact；文件先写同目录随机 temp、
  `Sync` 后原子 rename，JSON 固定缩进和尾随换行，权限不宽于 `0700/0600`。
- [ ] 测试并实现 plan SHA：对不含自身 hash 的稳定 JSON payload 计算 SHA-256；plan 显式
  路径可位于 caller cwd 下，但 apply 不查找“最新”文件。
- [ ] 测试 preview binding 原子写入 manifest，且 plan SHA、source/target identities、
  before hashes、template/scope 与 allow-remove 任一变化都产生明确 mismatch。
- [ ] 测试 manifest/失败文件不会序列化环境、token、cookie 或 command output 中的
  credential-shaped value；错误只保留类别和已清洗摘要。
- [ ] 运行 `go test -count=1 ./internal/evidence`，确认全部 GREEN。
...
- [ ] 先测试固定命令树 `plan` 与 `apply`，显式 source store/theme、成对 repeatable target
  store/theme、输入顺序、`--template`/`--page`、三种 scope 和只允许 field
  `custom_css`；数量不等或空值时 adapter call count 为零。
- [ ] `plan` 顺序执行 source resolve/read/parse，再为全部 targets resolve/Live check/read/
  parse/transform/diff；任一失败返回 `FAILED` 且 write count 为零。
- [ ] plan evidence 写 source before/canonical 和每 target before/planned/diff/manifest，状态仅
  `NO_OP` 或 `PLANNED`；target template not-found 不转成 create。
- [ ] 先测试任一 target 初始 role 为 Live 时所有 write 为零；source Live 仍可只读。
- [ ] `apply --plan <path>` 默认重做全部只读 preflight，与 plan identity/hash/scope/diff
  对比；一致时写 preview binding，返回 `NO_OP` 或 `PLANNED`，write count 为零。
- [ ] 先测试 source/target template 或 identity 漂移、plan 文件篡改、target 后序解析失败、
  removed 未授权都在全体 mutation 前失败。
- [ ] removed plan 允许生成和 preview；binding 中冻结 `allow_remove`，preview/execute 参数
  不一致时要求重新 preview。
- [ ] 运行
...
- [ ] 先测试 execute 没有 matching preview binding 时拒绝；no-op targets 保持 `NO_OP` 且
  永不 write。
- [ ] 先测试 execute 在任何 write 前重做全部 target preflight；后序 target failure 时
  write count 仍为零。
- [ ] 每个 READY target 写前再次 resolve，并精确比较 store/ID/name/role；role 变 Live、
  identity drift 或 before canonical hash drift 时停止且不得写该 target。
- [ ] WriteTemplate 成功后立即 ReadTemplate；只有 canonical hash 等于 planned hash 才标
  `APPLIED` 并进入下一 target。格式/comment 差异必须成功。
- [ ] 先测试两个及以上 targets 的 call sequence 为
  `resolve/read ... all-preflight -> resolve/read/write/read target-0 ->
  resolve/read/write/read target-1`，最大 write concurrency 为 1。
- [ ] 先测试首个 write failure => `FAILED`/`WRITE_FAILED`，首个 readback mismatch =>
  `READBACK_MISMATCH`，已有 APPLIED 后失败或 mismatch => `PARTIAL`，剩余 targets =>
  `SKIPPED_AFTER_FAILURE`。
- [ ] 写入后无自动 rollback/retry；manifest 保留 failure kind、write/readback evidence 和
  before/planned/after hash，after/failure artifact 在已知时立即落盘。
- [ ] `Run` 将 `NO_OP`/`PLANNED`/`APPLIED` 映射 exit 0，输入/安全错误映射 exit 2，
  `FAILED`/`READBACK_MISMATCH` 映射 exit 3，`PARTIAL` 映射 exit 4；结果 JSON 只输出一次。
- [ ] 运行
...
- [ ] SKILL.md 保持精简：何时使用、何时不要使用、默认 dry-run、plan/preview/execute
  授权、Live/Publish/theme create 禁止、多目标 preflight/readback、结果解释与 evidence 路径。
- [ ] CLI contract 列出通用 template、deprecated page alias、source/target pairing、scope、
  allow-remove、plan 路径、runtime root 和完整示例；不含真实 store/domain/theme ID。
- [ ] Result contract 列出 target/overall 状态、exit code、partial/readback mismatch、artifact
  authority 和明确 Cannot Claim。
- [ ] `agents/openai.yaml` display name 为 `Theme Template Sync`，description 明确适用于
  Shopify JSON templates，default prompt 使用 `$theme-template-sync`，metadata 不暗示官方
  Shopify 产品。
- [ ] copy-install 测试运行安装副本 wrapper `--help`，从 caller cwd 执行且不写 Skill
  安装目录；lock hash 用仓库 verifier 验证 `MATCH`。
- [ ] 运行
  `python3 /Users/julcyan/.codex/skills/.system/skill-creator/scripts/quick_validate.py skills/theme-template-sync`
...
- Inspect only:
  - `/Users/julcyan/Documents/Workspace/shopify-theme-multi/scripts/ops/sync-page.js`
  - `/Users/julcyan/Documents/Workspace/shopify-theme-multi/scripts/ops/lib/sync-page-core.js`
  - `/Users/julcyan/Documents/Workspace/shopify-theme-multi/scripts/ops/lib/page-json.js`
  - 同目录 audit/config/tests/callers

### Steps

- [ ] 对照迁移审计表确认直接迁移：template/page alias、前导 comment、Section replacement、
  source-predecessor order、stable diff/hash、removed gate、dry-run/execute、跨店/跨主题。
- [ ] 确认移除耦合：golden/all/exclude、真实 store/theme config、dotenv/secret helper、
  staging duplicate、Liquid/media/Git sync、业务 tmp/backup/log、shell command strings。
- [ ] 用合成 fixture 运行 `go test -count=1 -race ./...` 和 `go vet ./...`（新 module），
  再对旧 module 运行既有 `go test ./...`，不得把 baseline 失败误报为本 change 成功。
- [ ] 运行 `sh -n` 两个 wrapper、skill validator、Node tests、公开内容扫描、
  `OPENSPEC_TELEMETRY=0 npx --yes @fission-ai/openspec@1.7.0 validate --all --strict --no-interactive`
  与完整 `npm test`。
- [ ] 从干净 tracked snapshot copy-install 两个 Skills，确认新 Skill 在 provider 外 cwd
  `--help` 成功且安装目录无 runtime evidence。
- [ ] 检查 `git diff --check`、`git status --short`、tracked 文件名与公开扫描；确认没有
  `.env`、credential、真实 domain/theme ID、个人绝对路径、runtime evidence 或消费项目
  改动。
- [ ] 将所有已完成 checkbox 更新为 `[x]`；只记录实际通过的验证，Release、真实 Shopify、
